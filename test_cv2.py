"""
이미지에 사선이 생길 경우 아래의 코드로 수정이 가능했음

이미지가 꽉차있거나 한 쪽만 꽉차있으면 이미지가 잘 추출이 안 됨
때문에 이럴 경우 원본이미지에 패딩을 채운 후 이 패딩과 색상 차이는 나기 때문에 이미지 윤곽선을 딸 때 더 잘 가져올 수 있음

배경이 있는 경우, 배경의 윤곽선 때문에 배경까지 추려진다. 이럴 경우엔 커널값을 변경하여 고칠 수도 있지만
배경을 블러처리한 후 윤곽선을 따주는 것이 좋다.

contour = 색상이 있는 좌표값 = (x,y)
x가 안쪽 loop, y가 바깥쪽l loop
ex) (0,0) (1,0) (2,0) (3,0)
    (0,1) (1,1) (2,1) (3,1)
    (0,2) (1,2) (2,2) (3,2)
    (0,3) (1,3) (2,3) (3,3)

image = 픽셀 = 색상값(그레이는0~255, color는 RGB 각각이0~255)
"""
...
gray_img=cv.GaussianBlur(gray_img,(1,1),0) #이 커널값은 이미지에 따라 홀수로 수정하기 1,1이나 3,3이나 5,5처럼
