...
def get_naver(search_datas,save_directory,cnt_count):
    ...
    for searchKeyword,keyword in search_datas:
       ...
       #print(not not re.search(pattern=pattern, string=ele).groups()[7])for ele in f_ele) #php형태로 정해둔 규칙에 맞지 않아 false가 발생
       f_url = []
       for ele in f_ele:
           try:
       ...
       for u in f_url:
           ...
           try:
                global contentLen
                #content-lenght가 구글에선 됐는데, 네이버에선 Content-Length인 것들이 있어서 이렇게 적어줌
                contentLen = (img_site.headers["Content-Length"] or img_site.headers["content-length"])
           except:
               continue
